...
%     p = initschnitz(pos_names{i},date_in,'bacillus','rootDir',data_path{date_now},'imageDir',data_path{date_now});
    ...
    yreg=imread([data_path_raw{date_now},pos_names_data{i}(1:end-4),'_',do.rfp_name,'_s',num2str(i),'_t',num2str(L),'.tif']);
    im_out(:,:,1)=mat2gray(imp);
    im_out(:,:,2)=mat2gray(imp);
    im_out(:,:,3)=mat2gray(imp);
    %im_out(:,:,1)=mat2gray(rreg)+im_out(:,:,1);
    im_out(:,:,2)=mat2gray(yreg)+im_out(:,:,1);;
    

%     figure;
    imshow(im_out);
    text(800,50,pos_names{i},'FontSize',20,'color','r');
    saveas(gcf,['C:\Users\christian.schwall\Documents\tempfiles\2023-04-21\',pos_names{i},'-y.png']);
